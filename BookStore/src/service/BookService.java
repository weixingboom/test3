...
public class BookService {
    ...
    // 实现添加逻辑处理
    public boolean addBook(String id, String name,  int number,String address){
        // 遍历数据，判断要插入的水果编号是否存在
        ArrayList<Person> data=queryAll();
        for(int i=0;i<data.size();i++){
            Person book=data.get(i);
            if(id.equals(book.getId())){
                return false;
            }
        }
        Person thisbook=new Person(id,name,number,address);
     ...
     * @return
     */
    public boolean updateBook(String id, String name,  int number,String address) {
        // 遍历数据，判断要插入的号码编号是否存在
        ArrayList<Person> data=queryAll();
        for(int i=0;i<data.size();i++){
            Person book=data.get(i);
